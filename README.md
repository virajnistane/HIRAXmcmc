...
4. (Optional) Edit ```.env``` for custom uv install/cache paths and set ```UV_SYSTEM_SITE_PACKAGES=true``` to create ```.venv``` with system site packages
...
8. Install the built wheel: ```uv pip install dist/hiraxmcmc*.whl```
